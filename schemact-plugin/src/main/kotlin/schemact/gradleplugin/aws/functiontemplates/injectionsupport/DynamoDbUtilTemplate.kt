...
object DynamoDbUtilTemplate {

    fun DynamoDbUtilTemplate(packageName: String) = """    
package $packageName
//create by DynamoDbUtilTemplate
    ...
    fun <T> createOrUpdate(userTableName: String, userId: String, email: String, dataType: Class<T>, defaultData: ()-> T,
        ...
        try {
            ...
            if (existingData != null) {
                ...
                println("Updated user data in DynamoDB for user_id=${dollarChar}userId")
            } else {
                ...
                println("Created new user data in DynamoDB for user_id=${dollarChar}userId")

            }
            return data
            //context.logger.log("Stored user data in DynamoDB for user_id=${dollarChar}userId")
        } catch (e: Exception) {
            e.printStackTrace()
            println("oops DynamoDB write failed: ${dollarChar}{e.message}")
            throw e
            //context.logger.log("DynamoDB write failed: ${dollarChar}{e.message}")
