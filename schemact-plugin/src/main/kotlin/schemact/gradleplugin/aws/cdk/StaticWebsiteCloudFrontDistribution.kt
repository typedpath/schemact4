...
object CreateWebsiteResourcesCloudFrontDistribution {
    fun createWebsiteResourcesCloudFrontDistribution(
        scope: Construct, domain: Domain, websiteDomainName: String,
        idToFunctionUrl: Map<String, CfnUrl>
    ): CfnDistribution {
        val originConfigProperties = mutableListOf(createDefaultOriginProperty(websiteDomainName))
        originConfigProperties.addAll(idToFunctionUrl.map {
            createFunctionOriginProperty(
                it.key,
                it.value
            )
        })
    ...
    fun createFunctionOriginProperty(
        id: String,
        functionUrl: CfnUrl
